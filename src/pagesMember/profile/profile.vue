<script setup lang="ts">
import { ref } from 'vue'
import { getMemberProfileAPI, modifyMemberProfileleAPI } from '../../services/profile'
import type { ProfileDetail, Gender } from '../../types/member'
import { onLoad } from '@dcloudio/uni-app'
import { useMemberStore } from '@/stores'
// import UniHelper from '@uni-helper/uni-ui-types'
//
const { safeAreaInsets } = uni.getSystemInfoSync()

const profile = ref<ProfileDetail>({} as ProfileDetail)
const memberStore = useMemberStore()
onLoad(() => {
  getMemeberProfile()
})
const getMemeberProfile = async () => {
  const res = await getMemberProfileAPI()
  console.log(res, 'getMemeberProfile res')
  profile.value = res.result
}
const onAvatarChange = () => {
  //拍照 or 选图
  uni.chooseMedia({
    count: 1,
    mediaType: ['image'],
    success: (res) => {
      const { tempFilePath } = res.tempFiles[0]
      uni.uploadFile({
        url: '/member/profile/avatar',
        name: 'file',
        filePath: tempFilePath,
        success: (res) => {
          if (res.statusCode === 200) {
            const avatar = JSON.parse(res.data).result.avatar
            profile.value!.avatar = avatar
            memberStore.profile!.avatar = profile.value!.avatar
            uni.showToast({ title: '上传成功', icon: 'success' })
          } else {
            uni.showToast({ title: '上传失败', icon: 'error' })
          }
        },
      })
    },
  })
}

//保存
const onSaveProfile = async () => {
  console.log(modifyMemberProfileleAPI, 'modifyMemberProfileleAPI')
  const { nickname, gender, birthday, profession } = profile.value!
  const [provinceCode, cityCode, countyCode] = fullReginCode
  const res = await modifyMemberProfileleAPI({
    nickname,
    gender,
    birthday,
    provinceCode,
    cityCode,
    countyCode,
    profession,
  })
  if (res.code === '1') {
    uni.showToast({ title: '保存成功', icon: 'success' })
    memberStore.profile!.nickname = profile.value!.nickname
    setTimeout(() => {
      uni.navigateBack()
    }, 1000)
    console.log(res, 'res')
  } else {
    uni.showToast({ title: '保存失败', icon: 'fail' })
  }
  // memberStore.setProfile(Object.assign(memberStore.profile, {nickname:}))
}
const onGenderChange = (event: { detail: { value: Gender } }) => {
  profile.value.gender = event.detail.value
  // console.log(event.detail.value, 'event')
}
const onBirthdayChange = (event: { detail: { value: string } }) => {
  // console.log(event.detail.value, 'event.detail.value')
  profile.value.birthday = event.detail.value
}
let fullReginCode: [string, string, string] = ['', '', '']
const onReginChange = (event) => {
  profile!.value.fullLocation = event.detail.value.join(' ')
  fullReginCode = event.detail.code!
  // console.log(event.detail, 'event.detail.value')
}
const onProfessionChange = (event) => {
  console.log(event, 'event.detail')
  profile!.value.profession = event.detail.value
}
</script>
<template>
  <view class="viewport">
    <!-- 导航栏 -->
    <view class="navbar" :style="{ paddingTop: safeAreaInsets?.top + 'px' }">
      <navigator open-type="navigateBack" class="back icon-left" hover-class="none"></navigator>
      <view class="title">个人信息</view>
    </view>
    <!-- 头像 -->
    <view class="avatar">
      <view class="avatar-content">
        <image class="image" :src="profile?.avatar" mode="aspectFill" @tap="onAvatarChange" />
        <text class="text">点击修改头像</text>
      </view>
    </view>
    <!-- 表单 -->
    <view class="form">
      <!-- 表单内容 -->
      <view class="form-content">
        <view class="form-item">
          <text class="label">账号</text>
          <text class="account">{{ profile?.account }}</text>
        </view>
        <view class="form-item">
          <text class="label">昵称</text>
          <input class="input" type="text" placeholder="请填写昵称" v-model="profile!.nickname" />
        </view>
        <view class="form-item">
          <text class="label">性别</text>
          <!-- TODO:Type proptotypes list -->
          <radio-group @change="onGenderChange">
            <label class="radio">
              <radio value="男" color="#ec5514" :checked="profile?.gender === '男'" />
              男
            </label>
            <label class="radio">
              <radio value="女" color="#ec5514" :checked="profile?.gender === '女'" />
              女
            </label>
            <label class="radio">
              <radio value="保密" color="#ec5514" :checked="profile?.gender === '保密'" />保密
            </label>
          </radio-group>
        </view>
        <view class="form-item">
          <text class="label">生日</text>
          <picker
            @change="onBirthdayChange"
            class="picker"
            mode="date"
            start="1900-01-01"
            :end="new Date()"
            :value="profile?.birthday"
          >
            <view v-if="profile?.birthday">{{ profile?.birthday }}</view>
            <view class="placeholder" v-else>请选择日期</view>
          </picker>
        </view>
        <view class="form-item">
          <text class="label">城市</text>
          <picker
            @change="onReginChange"
            class="picker"
            mode="region"
            :value="profile?.fullLocation?.split(' ')"
          >
            <view v-if="false">{{ profile!.fullLocation }}</view>
            <view class="placeholder" v-else>请选择城市</view>
          </picker>
        </view>
        <view class="form-item">
          <text class="label">职业</text>
          <input
            @change="onProfessionChange"
            class="input"
            type="text"
            placeholder="请填写职业"
            v-model="profile!.profession"
          />
        </view>
      </view>
      <!-- 提交按钮 -->
      <button class="form-button" @tap="onSaveProfile">保 存</button>
    </view>
  </view>
</template>

<style lang="scss">
page {
  background-color: #f4f4f4;
}

.viewport {
  display: flex;
  flex-direction: column;
  height: 100%;
  background-image: url(../../static/images/order_bg.png);
  background-size: auto 420rpx;
  background-repeat: no-repeat;
}

// 导航栏
.navbar {
  position: relative;

  .title {
    height: 40px;
    display: flex;
    justify-content: center;
    align-items: center;
    font-size: 16px;
    font-weight: 500;
    color: #fff;
  }

  .back {
    position: absolute;
    height: 40px;
    width: 40px;
    left: 0;
    font-size: 20px;
    color: #fff;
    display: flex;
    justify-content: center;
    align-items: center;
  }
}

// 头像
.avatar {
  text-align: center;
  width: 100%;
  height: 260rpx;
  display: flex;
  flex-direction: column;
  justify-content: center;
  align-items: center;

  .image {
    width: 160rpx;
    height: 160rpx;
    border-radius: 50%;
    background-color: #eee;
  }

  .text {
    display: block;
    padding-top: 20rpx;
    line-height: 1;
    font-size: 26rpx;
    color: #fff;
  }
}

// 表单
.form {
  background-color: #f4f4f4;

  &-content {
    margin: 20rpx 20rpx 0;
    padding: 0 20rpx;
    border-radius: 10rpx;
    background-color: #fff;
  }

  &-item {
    display: flex;
    height: 96rpx;
    line-height: 46rpx;
    padding: 25rpx 10rpx;
    background-color: #fff;
    font-size: 28rpx;
    border-bottom: 1rpx solid #ddd;

    &:last-child {
      border: none;
    }

    .label {
      width: 180rpx;
      color: #333;
    }

    .account {
      color: #666;
    }

    .input {
      flex: 1;
      display: block;
      height: 46rpx;
    }

    .radio {
      margin-right: 20rpx;
    }

    .picker {
      flex: 1;
    }
    .placeholder {
      color: #808080;
    }
  }

  &-button {
    height: 80rpx;
    text-align: center;
    line-height: 80rpx;
    margin: 30rpx 20rpx;
    color: #fff;
    border-radius: 80rpx;
    font-size: 30rpx;
    background-color: #ec5514;
  }
}
</style>
